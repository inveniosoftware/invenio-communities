...
def test_update_cant_update_other_than_payload(
        community_creation_input_data, community_service,
        create_community_identity, create_user_identity,
        generate_invitation_input_data, make_member_identity):
    owner_identity = create_user_identity("owner@example.com")
    ...
    data = generate_invitation_input_data(
        community.id, {"user": str(user_identity.id)}, "reader"
    )
    invitation = community_service.invitations.create(owner_identity, data)

    # Can modify role
    data_copy = deepcopy(invitation.data)
    data_copy["payload"]["role"] = "curator"
    updated_invitation = community_service.invitations.update(
        owner_identity, invitation.id, data_copy
    )
    assert invitation.id == updated_invitation.id
    assert "curator" == updated_invitation.data["payload"]["role"]

    # Can't modify type
    data_copy = deepcopy(invitation.data)
    data_copy["type"] = "my-custom-other-type"
    with pytest.raises(ValidationError):
        community_service.invitations.update(
            owner_identity, invitation.id, data_copy
        )

    # Can't modify receiver
    data_copy = deepcopy(invitation.data)
    data_copy["receiver"] = {"user": "1234"}
    with pytest.raises(ValidationError):
        community_service.invitations.update(
            owner_identity, invitation.id, data_copy
        )
    ...
    # Can't modify topic
    data_copy = deepcopy(invitation.data)
    data_copy["topic"] = {"community": "4321"}
    with pytest.raises(ValidationError):
        community_service.invitations.update(
            owner_identity, invitation.id, data_copy
        )
