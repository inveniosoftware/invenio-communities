...
# Add members
#
@pytest.mark.parametrize("actor,role", [
    ('owner', 'owner'),
    ('owner', 'manager'),
    ('owner', 'curator'),
    ('owner', 'reader'),
...
def test_add_denied(
        member_service, community, members, group, actor, role, db):
...
def test_add_invalid_member_type(
...
def test_invite_group_denied(
    ...
    assert pytest.raises(
        InvalidMemberError,
        member_service.invite, system_identity, community._record.id, data
    )
...
def test_search_members_restricted(
        member_service, restricted_community, owner, anon_identity,
        clean_index):
...
def test_invite_cancel_flow(
        member_service, requests_service, community, owner, invite_request_id,
        db, clean_index):
    """Invite a user."""
    # Cancel request
    request = requests_service.execute_action(
        owner.identity, invite_request_id, 'cancel').to_dict()
...
def test_invite_actions_permissions(
        requests_service, owner, any_user, members, invite_user,
        invite_request_id, db):
    ...
    for user in [owner, any_user, manager, curator, reader]:
        pytest.raises(
            PermissionDeniedError,
            requests_service.execute_action,
            user.identity, invite_request_id, 'accept'
        )
        pytest.raises(
            PermissionDeniedError,
            requests_service.execute_action,
            user.identity, invite_request_id, 'decline'
        )
    ...
    for user in [any_user, curator, reader, invite_user]:
        pytest.raises(
            PermissionDeniedError,
            requests_service.execute_action,
            user.identity, invite_request_id, 'cancel'
        )
...
def test_delete_denied(
        member_service, community, members, actor, new_user, role, db):
...
def test_delete_member_type_group(
        member_service, community, owner, group, db):
...
def test_selfupdate_denied(member_service, community, any_user, new_user):
    ...
    for u in [any_user, new_user]:
        pytest.raises(
            PermissionDeniedError,
            member_service.update, u.identity, community._record.id, data
        )


@pytest.mark.parametrize("actor", [
    'owner', 'manager', 'curator', 'reader',
])
...
def test_selfupdate_allow_visibility(
        member_service, community, members, actor, db):
...
def test_update_invalid_data(member_service, community, group):
    ...
    # Invalid type
    data = {
        "members": [{"type": "email", "id": group.name}],
        "role": "owner"
    }
    pytest.raises(
        InvalidMemberError,
    ...
    data = {
        "members": [{"type": "invalid", "id": group.name}],
        "role": "owner"
    }
    pytest.raises(
        ValidationError,
