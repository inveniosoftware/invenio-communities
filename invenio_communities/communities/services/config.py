...
from invenio_records_resources.services.files.links import FileLink2
...
from .links import CommunityLink2, CommunityUILink2
...
class CommunityServiceConfig(RecordServiceConfig, ConfiguratorMixin):
    ...
    links_item = {
        "self": CommunityLink2("communities.read", params=["pid_value"]),
        "self_html": CommunityUILink2(
            "invenio_app_rdm_communities.communities_home", params=["pid_value"]
        ),
        "settings_html": CommunityUILink2(
            "invenio_communities.communities_settings", params=["pid_value"]
        ),
        "logo": CommunityLink2("communities.read_logo", params=["pid_value"]),
        "rename": CommunityLink2("communities.rename", params=["pid_value"]),
        "members": CommunityLink2("community_members.search", params=["pid_value"]),
        "public_members": CommunityLink2(
            "community_members.search_public", params=["pid_value"]
        ),
        "invitations": CommunityLink2("community_members.invite", params=["pid_value"]),
        "requests": CommunityLink2(
            "communities.search_community_requests", params=["pid_value"]
        ),
        "records": CommunityLink2("community-records.search", params=["pid_value"]),
        "subcommunities": CommunityLink2(
            "communities.search_subcommunities",
            when=children_allowed,
            params=["pid_value"],
        ),
        "membership_requests": CommunityLink2(
            "community_members.request_membership", params=["pid_value"]
        ),
    }

    action_link = CommunityLink2(
        "communities.featured_create",  # This works because only 1 action
        when=can_perform_action,
        params=["pid_value"],
    )

    links_search = pagination_links2("communities.search")
    links_featured_search = pagination_links2("communities.featured_communities_search")
    links_user_search = pagination_links2("communities.search_user_communities")
    links_community_requests_search = pagination_links2(
        "communities.search_community_requests", params=["pid_value"]
    )
    links_subcommunities_search = pagination_links2(
...
class CommunityFileServiceConfig(FileServiceConfig, ConfiguratorMixin):
    ...
    file_links_item = {
        "self": FileLink2("communities.read_logo", params=["pid_value"]),
    }
