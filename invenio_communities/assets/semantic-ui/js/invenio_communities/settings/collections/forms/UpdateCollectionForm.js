...
class UpdateCollectionForm extends Component {
  ...
  onTest = async (values) => {
    const { collectionTreeSlug, collectionSlug } = this.props;
  ...
  onSubmit = async (values, { setSubmitting, setFieldError }) => {
    ...
    try {
      await this.props.collectionApi.update_collection(
        collectionTreeSlug,
        collectionSlug,
        payload
      );
  ...
  render() {
    const { slugGeneration, community, parentQuery, onFormReady } = this.props;
